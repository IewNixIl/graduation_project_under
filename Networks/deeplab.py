...
class _ResLayer(nn.Sequential): 
    ...
    def __init__(self, n_layers, in_ch, out_ch, stride, dilation, multi_grids=None): 
        ...
        else: assert n_layers == len(multi_grids) # Downsampling is only in the first block 
...
class _Stem(nn.Sequential): 
    """ The 1st conv layer. Note that the max pooling is different from both MSRA and FAIR ResNet. """ 
...
if __name__ == "__main__": 
    model = DeepLabV3( n_classes=21, n_blocks=[3, 4, 23, 3], atrous_rates=[6, 12, 18], multi_grids=[1, 2, 4], output_stride=8, ) 
    model.eval() 
