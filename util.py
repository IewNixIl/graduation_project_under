...
def segment(from_path,to_path,thre):
    ...
    for i in range(len(d)):
        predict=numpy.array(Image.open(from_path+'\\'+d[i]))/255
        print(predict.max())
        img=numpy.where(predict>thre,255,0).astype(numpy.uint8)
...
def merge_segement(from_paths,to_path,thres):
    ...
    for i in range(len(d)):
        ...
        for j in range(len(from_paths)):
            ...
            img=numpy.where(predict>thres[j],1,0)
            sta=sta+img
...
def mask_another(from_path,to_path):
    '''生成mask，对于不确定的像素，赋予0，确定的赋予1
    从merge中获得
    保存0-1
    '''
...
if __name__=='__main__':
    path_predict_model1='D:\\result\\rest_result\\5ite\\model1'
    path_predict_model2='D:\\result\\rest_result\\5ite\\model2'
    path_predict_model3='D:\\result\\rest_result\\5ite\\model3'
    path_merge='D:\\result\\rest_result\\1ite\\merge'
    path_merge_correct='D:\\result\\rest_result\\1ite\\merge_correct'
    path_random='D:\\result\\rest_result\\1ite\\merge_correct_random'
    path_label='D:\\labels\\labels'
    path_mask='D:\\result\\rest_result\\1ite\\mask_correct'
    ...
    path_mean='D:\\result\\rest_result\\5ite\\mean'
    path_mean_seg='D:\\result\\rest_result\\5ite\\mean_seg'
    thres=[0.32,0.47,0.61]
    window_size=5

    segment('D:\\result\\final_result\\ndwi','D:\\result\\final_result\\ndwi_seg',0.01)
    #random_plus_merge(path_merge,path_random,path_random_merge)
    #masked_label(path_merge,path_mask,path_merge_mask)
    #segment('D:\\result\\rest_result\\5ite\\mean','D:\\result\\rest_result\\5ite\\mean_seg',0.38)
    #err=caculate_square_error(path_merge_correct,path_label)#,path_mask)
    #print(err)
    ...
    #random_label(path_merge_correct,path_random)
    '''
